...
data = pd.read_json('data_new.json')
...
for i in np.arange(0, rows):
    ...
    author_string = ""
    for j in range(len(x[i][0]['authors'])):
        author_string += (x[i][0]['authors'][j] + " ")

    # need to create a new string based on frequency
    abs_string = ""
    for k in x[i][0]['word_count']:
        if k not in common:
            for l in range(int(x[i][0]['word_count'][k])):
                abs_string += (k + " ")

    new_title = x[i][0]['title']

    final = ""
    for val in range(5):
        final += x[i][0]['journal'] + " " + author_string + " " + new_title + " "
    ...
    arr.append(final)
    new_row = {'title': new_title, 'authors': x[i][0]['authors']}
...
choices = pd.read_json('survey.json')
author_name = pd.read_json('author_name.json')
...
for index, row in df.iterrows():
    if row['cluster'] == cluster_pref and any(x in author_pref for x in row['authors']):
        # add it to results
